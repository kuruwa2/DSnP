...
CmdParser::readCmdInt(istream& istr)
{
	...
	while (1) {
			...
			switch (pch) {
				...
				case TAB_KEY        : /* TODO */
					int count;
					count = TAB_POSITION - (_readBufPtr - _readBuf) % TAB_POSITION;
					insertChar(' ', count);
					break;
...
CmdParser::deleteChar()
{
	...
	if (_readBufPtr == _readBufEnd){
		...
		for (i = _readBufPtr; i < _readBufEnd; i++)
			*i = *(i + 1);
		_readBufEnd --;
		moveBufPtr(_readBufEnd);
		cout << "\33[2K\r" << "cmd> " << _readBuf;
		moveBufPtr(ptr);
		return true;
	}
}
...
CmdParser::insertChar(char ch, int repeat)
{
	...
	while(repeat > 0)
	{
		char* i;
		_readBufEnd ++;
		for (i = _readBufEnd; i > _readBufPtr; i--)
			*i = *(i - 1);
		*i = ch;
		moveBufPtr(_readBufEnd);
		cout << "\33[2K\r" << "cmd> " << _readBuf;
...
CmdParser::moveToHistory(int index)
{
	...
	if (_historyIdx == size){
	 	...
	 	_tempCmdStored = true;
		size ++;
	}else if (_historyIdx == size - 1)
	 	_history[size - 1] = _readBuf;

	if (index < _historyIdx && _historyIdx == 0)
	 	mybeep();
	else if (index > _historyIdx && _historyIdx == size - 1)
	 	mybeep();
	else{
		if (index >= size)
			index = size - 1;
...
CmdParser::addHistory()
{
	...
	if (temp.find_first_not_of(' ') < temp.size()){
		...
		temp.erase(temp.begin() + temp.find_last_not_of(' ') + 1, temp.end());
		if (_tempCmdStored)
			_history[_history.size() - 1] = temp;
		else
