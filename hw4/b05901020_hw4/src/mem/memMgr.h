...
class MemRecycleList
{
   ...
   void reset() {
      // TODO
        MemRecycleList<T>* doki = getNextList();
        MemRecycleList<T>* club;
        while (doki){
            club = doki -> getNextList();
            doki -> setNextList(0);
            delete doki;
            doki = club;
        }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
        MemBlock<T>* doki = _activeBlock;
        MemBlock<T>* club;
        while (doki -> getNextBlock()){
            club = doki -> getNextBlock();
            delete doki;
            doki = club;
        }
        _activeBlock = doki;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
        MemRecycleList<T>* MRL = &_recycleList[m];
        while(true){
            if (MRL -> getArrSize() == n)
                return MRL;
            if (MRL -> getNextList() == 0){
                MRL -> setNextList(new MemRecycleList<T> (n));
                return MRL -> getNextList();
            }
            MRL = MRL -> getNextList();
        }
   }
   ...
   T* getMem(size_t t) {
      ...
      //    #endif // MEM_DEBUG
        ...
        MemRecycleList<T>* MRL = getMemRecycleList(n);
        if (MRL -> _first != 0){
            ret = MRL -> popFront();
            #ifdef MEM_DEBUG
            cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
