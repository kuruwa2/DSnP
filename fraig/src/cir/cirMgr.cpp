...
CirMgr::printPIs() const
{
   ...
   for (size_t i = 0, s = PIs.size(); i < s; ++i)
        cout << " " << PIs[i]/2;
   cout << endl;
}
...
CirMgr::printPOs() const
{
   ...
   for (size_t i = 0, s = POs.size(); i < s; ++i)
       cout << " " << MILOA[0] + i + 1;
   cout << endl;
}
...
CirMgr::printFECPairs() const
{
    for(size_t i = 0, s = _fecGrps.size(); i < s; ++i)
    {
        ...
        {
            cout << " ";
            if(_gate[(*_fecGrps[i])[j]] -> getSimInv())
                cout << '!';
            cout << (*_fecGrps[i])[j];
        }
        cout << endl;
...
CirMgr::writeAag(ostream& outfile) const
{
    ...
    for(size_t i = 0, s = POs.size(); i < s; ++i)
        outfile << POs[i] << endl;
    for(size_t i = 0, s = dfsAigs.size(); i < s; ++i){
    ...
    outfile << "c" << endl;
    outfile << "AAG output by Kuruwa" << endl;
}

void
CirMgr::writeGate(ostream& outfile, CirGate *g) const
{
    vector<string> fanin, syms, aigs;
    vector<string>::iterator it;
    int index = 0;
    CirGate::setGlobal();
	unsigned vars = g -> getCone(&fanin, &aigs);
	outfile << "aag " << vars << ' ' << fanin.size() << " 0 1 " << aigs.size() << '\n';
	for (int i = 0; i < MILOA[1]; ++i){
        it = find(fanin.begin(), fanin.end(), std::to_string(PIs[i]));
        if (it != fanin.end()){
            outfile << PIs[i] << endl;
            if (_gate[PIs[i]/2] -> getSymbol() != "")
                syms.push_back("i" + std::to_string(index) + ' ' + _gate[PIs[i]/2] -> getSymbol());
            ++ index;
        }
    }
	outfile << 2 * g -> id() << endl;
	for (unsigned i = 0, size = aigs.size(); i < size; ++i)
		outfile << aigs[i] << endl;
	for (unsigned i = 0, size = syms.size(); i < size; ++i)
		outfile << syms[i] << endl;
    outfile << "o0 " << g -> id() << endl;
	outfile << "c\nWrite gate (" << vars << ") by Kuruwa\n";
}
...
CirMgr::parsepo(ifstream& file)
{
    ...
    for(int i = 0; i < MILOA[3]; ++i){
        ...
        dynamic_cast<CirPoGate*>(_gate[MILOA[0]+i+1]) -> setId(i);
        ++lineNo;
    }
    return true;
}
